...
$('#waitingList .patientWaitingList').click(handleClick);
	...
	$waitingList.on("click", ".patientWaitingList", function() {
		var rceptNo = $(this).find("button").val();
		console.log("제발 가져와라 접수 번호 : " + rceptNo);
		
		var status = $(this).find("button").text().trim();
		console.log("status : "+ status);
		// 환자 상태가 진료대기, 검사대기, 호출이 아니라면 이벤트를 막는기능
		if(status === "진료대기" || status === "검사대기" || status === "호출"){
			...
			// 환자 진료기록 조회
			selectAllClinic(_patntNo, rceptNo);
			// 환자 검사 결과 조회
			selectMEOResult(_patntNo, rceptNo);
			// 환자 엑스레이 결과 조회
			selectXrayResult(_patntNo, rceptNo);
		}
