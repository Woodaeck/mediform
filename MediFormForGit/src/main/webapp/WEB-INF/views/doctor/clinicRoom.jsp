			...
			<div class="card-header p-1" style="border-bottom: 1px solid #ededed; background-color: midnightblue;">
				...
				<div class="row">
					...
					<div class="col-2 ps-0" style="display: flex; align-items: center; justify-content: flex-end;">
						<button id="cleanBtn" class="btn btn-falcon-default btn-small resetButton py-1 px-2 me-1" type="button" style="font-size: 12px;">초기화</button>
					</div>
				</div>
				<!-- 환자 검색 끝 -->
			</div>
				...
				// 검사 완료된 사람
					...
					if(res.length == 0){ // 검사 결과가 없을때
						...
						for(var j = 0; j < res.length; j++){
							...
							xrayADivResultHtml += "<img alt='' src='"+res[j].fileFlpth+"' style='width: 480px; height: 600px; position: relative;'/>";
...
// 상병 관련 스크립트 시작=======================================================================================================
	...
	// 상병 검색 시작
		...
		function selectSAWLAjax(sickcd, krnm, sicknm, msick, sckwndSe){
			...
			sawlInsertTableHtml += "<div class='form-check form-check-sm form-check-inline pb-0 mb-0 me-0'>"
									+ "<input type='radio' id='sickRadio"+radioNo1+"' name='sickDivision"+number+"' class='form-check-input mainSick sickRadio' value='1'";
			if(msick == "N"){
			...
			sawlInsertTableHtml	+= "</div>";
			sawlInsertTableHtml	+= "<div class='form-check form-check-sm form-check-inline pb-0 mb-0 me-0'>";
			sawlInsertTableHtml	+= "<input type='radio' id='sickRadio"+radioNo2+"' name='sickDivision"+number+"' class='otherSick form-check-input sickRadio' value='2' "+checked2+">";
			sawlInsertTableHtml	+= "<label class='form-check-label ms-n1 mb-0' for='sickRadio"+radioNo2+"'>부</label>";
			sawlInsertTableHtml	+= "</div>";
			sawlInsertTableHtml	+= "<div class='form-check form-check-sm form-check-inline pb-0 mb-0 me-0'>";
			sawlInsertTableHtml	+= "<input type='radio' id='sickRadio"+radioNo3+"' name='sickDivision"+number+"' class='otherSick form-check-input sickRadio' value='3' "+checked3+">";
	...
	// 상병 검색 끝
		...
		$(".sawDelTable").on("change", ".sickRadio", function(){
...
// 				var sickRadio = $(".sickRadio:checked");
// 				// 라디오 버튼 중 체크되어 있는 곳의 name값을 map이라는 배열에 담아 반환한다.
// 				var sickRadioNm = $(".sickRadio:checked").map(function() {
// 					  return $(this).attr("name");
// 				}).get();
				
				// 이전 주상병 체크된 곳이 비어 있지 않고 방금 선택된 name 값이 아닐 때
				if(pastSelectNm !== null && pastSelectNm !== "" && pastSelectNm !== selectNm){
					// 이전 주상병 체크된 곳이 현재도 주상병에 체크 되어 있는 지 확인
					if($('input[name="'+pastSelectNm+'"]:eq(0)').prop('checked')){
						// 이전 주상병 체크된 곳의 부상병 라디오 버튼
						var beforeSelect = $('input[name="'+pastSelectNm+'"]:eq(1)');
						// 부상병 라디오 버튼 체크
...
// 캔버스 시작 ====================================================================================================================================
	...
	$("#tab-xray").on("click",".xrayImages", function(){
		$("#xrayUlTab").empty();
		$("#xrayDivTab").empty();
		...
		for(var j = 0; j < xrays.length; j++){
			...
			xrayDivResultHtml += "<div style='display: flex; justify-content: center;'>";
			xrayDivResultHtml += "<img alt='' src='"+xraySrc+"' style='width: 480px; height: 600px; position: relative; z-index: 1;' tab-xray/>";
			xrayDivResultHtml += "</div></div>";
		}
...
//	    lineCap 캔버스 펜 끝 모양 바꾸기
	   ...
	   $(document).on("keydown", function(event){
	    ...
	    // 캔버스 전체 클린
	    $("#canvasCleanBtn").on("click", function(){
